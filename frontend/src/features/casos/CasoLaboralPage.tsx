...
import { StepperVisualLaboral } from './components/StepperVisualLaboral';
...
export default function CasoLaboralPage() {
  ...
  const { etapas } = useEtapasArea('LABORAL');
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
          <div
            style={{
              width: 46, height: 46, borderRadius: 10, background: '#E8EDF8',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 14, fontWeight: 700, color: '#1B3A6B', flexShrink: 0,
            }}
          >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <Badge bg="#E8EDF8" color="#1B3A6B">Laboral</Badge>
              {caso.etapa_actual && (
                <Badge bg="#E8EDF8" color="#1B3A6B">{caso.etapa_actual.nombre}</Badge>
              )}
        ...
        {/* Stepper visual bifurcado — data-driven (ADR-0008) */}
        {caso.etapa_actual && etapas.length > 0 && (
          <StepperVisualLaboral etapas={etapas} etapaActualId={caso.etapa_actual_id} />
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', gap: 7,
            background: '#FFFFFF', color: '#1B3A6B', border: '1.5px solid #1B3A6B',
            borderRadius: 8, padding: '10px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer',
          }}
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#FFFFFF', borderRadius: 12, border: '1px solid #E5E2D8', padding: 22 }}>
            <h3 style={{ fontSize: 13, fontWeight: 700, color: '#1B3A6B', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 16, margin: '0 0 16px' }}>
              Datos del caso
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {[
                ...
                ...(caso.ficha ? [
                  ...
                  { label: 'Remuneración', value: caso.ficha.remuneracion ? `$${caso.ficha.remuneracion}` : '—' },
                ] : []),
              ].map((item, i) => (
                <div key={i}>
            ...
            {(() => {
              ...
              return (
                <a
                  ...
                  title={!telRaw ? 'Sin celular registrado' : !waUrl ? 'Número inválido para WhatsApp' : 'Abrir chat de WhatsApp'}
                  style={{
                    display: 'inline-flex', alignItems: 'center', gap: 7,
                    background: waUrl ? '#25D366' : '#C5CDD8',
                    color: '#FFFFFF', borderRadius: 8, padding: '9px 14px',
                    fontSize: 13, fontWeight: 600, textDecoration: 'none',
                    cursor: waUrl ? 'pointer' : 'not-allowed', opacity: waUrl ? 1 : 0.6,
                    userSelect: 'none', marginTop: 14, width: '100%',
