"""
Fixtures para tests del feature usuarios.

Estructura:
  usuarios_db_engine — engine con tablas usuario + refresh_token (session scope)
...
# ── Engine con tablas de usuarios ─────────────────────────────────────────────


@pytest.fixture(scope="session")
def usuarios_db_engine(db_url):
    """Engine con las tablas usuario + refresh_token (para auth de los tests).
    ...
    with engine.begin() as conn:
        ...
        # Tabla refresh_token (necesaria para que auth funcione en tests de integración)
        conn.execute(text("""
...
def db_session(usuarios_db_engine) -> Session:
    ...
    session.close()
    with usuarios_db_engine.begin() as conn:
        conn.execute(
            text("TRUNCATE refresh_token, usuario RESTART IDENTITY CASCADE")
        )
...
@pytest.fixture(scope="session")
def _test_app():
    """App FastAPI de test con el router de usuarios y auth registrados."""
    ...
    _app = FastAPI(title="Iuris Test — Usuarios")
